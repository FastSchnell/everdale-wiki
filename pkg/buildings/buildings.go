...
func GetDetail(name string) ([]*BuildDetail, error) {
	csvFile, err := os.Open(fmt.Sprintf("%s/logic/buildings.csv", config.CfgInstance.Everdale.Path))
	if err != nil {
		return nil, err
	}
	defer csvFile.Close()
	...
	for {
		...
		if cName == name {
			...
			bd.Name = cName
			bd.Level = cLevel
			setVal(row[7], row[8], bd)
			setVal(row[9], row[10], bd)
			setVal(row[11], row[12], bd)
			bds = append(bds, bd)
		}
